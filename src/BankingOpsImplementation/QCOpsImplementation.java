...
public class QCOpsImplementation extends BankingOperationsPOA implements Runnable{
    ...
    public synchronized boolean createAccountRecord(String firstName, String lastName, String address, String phone, String branch) {
        ...
        if (noErrorCount > 0) {
            ...
            try {
                ...
                generateLogFileServer(msg, "Server" , "QCServer");
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    public  synchronized  String[] getAccountCount() {
        ...
        allBranchCount.put("QC", hashArraylistSize());
        ...
        for (String key : allBranchCount.keySet()) {
            String temp = key + " " + allBranchCount.get(key);
            countOfCustomer[i] = temp;
            i++;
        }

        System.out.println("-----------Printing Array of Strings---------------");

        for (int j =0; j < countOfCustomer.length; j++){

            System.out.println(countOfCustomer[j]);
